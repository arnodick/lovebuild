REM this script makes a folder containing your ready-to-play game, and a zip file ready for distribution, in whatever folder it is run
REM the folder it creates will contain an exe file and all the required dlls, and the zip file will contain that same folder
...
powershell.exe -nologo -noprofile -command "& { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::CreateFromDirectory($env:GAMENAME, $env:GAMENAME + '.zip', 'Optimal', 'true'); }"